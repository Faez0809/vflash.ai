...
def block_content(context, missing=missing):
    ...
    yield ' rows</span>\n            </summary>\n            <form method="get" class="grid gap-3 border-b border-white/5 px-6 py-4 md:grid-cols-[1fr_180px_180px_auto]">\n                <input name="enrichment_q" value="'
    yield escape((undefined(name='enrichment_q') if l_0_enrichment_q is missing else l_0_enrichment_q))
    yield '" class="rounded-xl border border-white/10 bg-[#0d0f0f] px-3 py-3 text-sm text-[#e2e2e2] outline-none" placeholder="Search enrichment cache">\n                <select name="level_filter" class="rounded-xl border border-white/10 bg-[#0d0f0f] px-3 py-3 text-sm text-[#e2e2e2] outline-none">\n                    <option value="">All levels</option>\n                    '
    for l_1_level in ['intermediate', 'upper_intermediate', 'advanced']:
        l_1_level_filter = resolve('level_filter')
        _loop_vars = {}
        pass
        yield '\n                    <option value="'
        yield escape(l_1_level)
        yield '" '
        if ((undefined(name='level_filter') if l_1_level_filter is missing else l_1_level_filter) == l_1_level):
            pass
            yield 'selected'
        yield '>'
        yield escape(t_4(t_3(context.eval_ctx, l_1_level, '_', ' ')))
        yield '</option>\n                    '
    l_1_level = l_1_level_filter = missing
    yield '\n                </select>\n                <select name="quality_filter" class="rounded-xl border border-white/10 bg-[#0d0f0f] px-3 py-3 text-sm text-[#e2e2e2] outline-none">\n                    <option value="">All quality states</option>\n                    '
    for (l_1_value, l_1_label) in [('flagged', 'Flagged Only'), ('low_quality', 'Low Quality'), ('never_audited', 'Never Audited'), ('recently_audited', 'Recently Audited'), ('corrected', 'Corrected Words'), ('regenerated', 'Regenerated')]:
        l_1_quality_filter = resolve('quality_filter')
        _loop_vars = {}
        pass
        yield '\n                    <option value="'
        yield escape(l_1_value)
        yield '" '
        if ((undefined(name='quality_filter') if l_1_quality_filter is missing else l_1_quality_filter) == l_1_value):
            pass
            yield 'selected'
        yield '>'
        yield escape(l_1_label)
        yield '</option>\n                    '
    l_1_value = l_1_label = l_1_quality_filter = missing
    ...
    for l_1_enrichment in (undefined(name='enrichment_rows') if l_0_enrichment_rows is missing else l_0_enrichment_rows):
        ...
        yield '</p>\n                                '
        if environment.getattr(environment.getattr(l_1_enrichment, 'vocabulary'), 'needs_admin_review'):
            pass
            yield '\n                                <p class="text-xs text-[#ffb4ab] mt-2 mb-0">'
            yield escape((environment.getattr(environment.getattr(l_1_enrichment, 'vocabulary'), 'review_reason') or 'Needs review'))
            yield '</p>\n                                '
        yield '\n                                '
        if environment.getattr(environment.getattr(l_1_enrichment, 'vocabulary'), 'original_word'):
            pass
            yield '\n                                <p class="text-[10px] text-[#8a9291] mt-2 mb-0">Original: '
            yield escape(environment.getattr(environment.getattr(l_1_enrichment, 'vocabulary'), 'original_word'))
            yield '</p>\n                                '
        yield '\n                                '
        ...
        yield '\n                                <p class="text-[10px] text-[#8a9291] mt-2 mb-0">Audited: '
        yield escape((context.call(environment.getattr(environment.getattr(l_1_enrichment, 'last_audited_at'), 'strftime'), '%Y-%m-%d %H:%M', _loop_vars=_loop_vars) if environment.getattr(l_1_enrichment, 'last_audited_at') else 'never'))
        yield '</p>\n                                <p class="text-[10px] text-[#8a9291] mt-1 mb-0">Regenerated: '
        yield escape((context.call(environment.getattr(environment.getattr(l_1_enrichment, 'last_regenerated_at'), 'strftime'), '%Y-%m-%d %H:%M', _loop_vars=_loop_vars) if environment.getattr(l_1_enrichment, 'last_regenerated_at') else 'never'))
        yield '</p>\n                                '
        if environment.getattr(environment.getattr(l_1_enrichment, 'vocabulary'), 'corrected_at'):
            pass
            yield '\n                                <p class="text-[10px] text-[#8a9291] mt-1 mb-0">Corrected: '
            yield escape(context.call(environment.getattr(environment.getattr(environment.getattr(l_1_enrichment, 'vocabulary'), 'corrected_at'), 'strftime'), '%Y-%m-%d %H:%M', _loop_vars=_loop_vars))
            yield '</p>\n                                '
        ...
        yield escape(environment.getattr(environment.getattr(l_1_enrichment, 'vocabulary'), 'word'))
        yield '" class="rounded-xl border border-white/10 bg-[#0d0f0f] px-3 py-2 text-xs text-[#e2e2e2] outline-none" placeholder="Vocabulary word">\n                                    <select name="level" class="rounded-xl border border-white/10 bg-[#0d0f0f] px-3 py-2 text-xs text-[#e2e2e2] outline-none">\n                                        '
        for l_2_level in ['intermediate', 'upper_intermediate', 'advanced']:
            ...
            yield '" '
            if (environment.getattr(environment.getattr(l_1_enrichment, 'vocabulary'), 'level') == l_2_level):
                pass
                yield 'selected'
            yield '>'
            yield escape(t_4(t_3(context.eval_ctx, l_2_level, '_', ' ')))
            yield '</option>\n                                        '
        l_2_level = missing
        ...
        yield '</textarea>\n                                    <input name="bangla_meaning" value="'
        yield escape((environment.getattr(l_1_enrichment, 'bangla_meaning') or ''))
        yield '" class="rounded-xl border border-white/10 bg-[#0d0f0f] px-3 py-2 text-xs text-[#e2e2e2] outline-none" placeholder="Bangla meaning">\n                                    <input name="pronunciation" value="'
        ...
        yield '" class="rounded-xl border border-white/10 bg-[#0d0f0f] px-3 py-2 text-xs text-[#e2e2e2] outline-none" placeholder="Antonym">\n                                    <textarea name="example_sentence" rows="2" class="col-span-2 rounded-xl border border-white/10 bg-[#0d0f0f] px-3 py-2 text-xs text-[#e2e2e2] outline-none" placeholder="Example sentence">'
        yield escape((environment.getattr(l_1_enrichment, 'example_sentence') or ''))
        yield '</textarea>\n                                    <label class="col-span-2 flex items-center gap-2 text-xs text-[#c0c8c7]">\n                                        <input type="checkbox" name="needs_admin_review" value="1" '
        if environment.getattr(environment.getattr(l_1_enrichment, 'vocabulary'), 'needs_admin_review'):
            pass
            yield 'checked'
        yield '>\n                                        Needs admin review\n                                    </label>\n                                    <input name="review_reason" value="'
        yield escape((environment.getattr(environment.getattr(l_1_enrichment, 'vocabulary'), 'review_reason') or ''))
        yield '" class="col-span-2 rounded-xl border border-white/10 bg-[#0d0f0f] px-3 py-2 text-xs text-[#e2e2e2] outline-none" placeholder="Review reason">\n                                    <button type="submit" class="col-span-2 rounded-xl bg-[#2d5f5d] px-3 py-2 text-[10px] font-bold uppercase tracking-[0.18em] text-[#b9ece9] border-none cursor-pointer">Save Manual Correction</button>\n                                </form>\n                            </td>\n                            <td class="px-6 py-5">\n                                <form method="post" action="'
        yield escape(context.call((undefined(name='url_for') if l_0_url_for is missing else l_0_url_for), 'admin_regenerate_enrichment', enrichment_id=environment.getattr(l_1_enrichment, 'id'), _loop_vars=_loop_vars))
        yield '" class="m-0 grid gap-2">\n                                    <input type="hidden" name="csrf_token" value="'
        yield escape((undefined(name='admin_csrf_token') if l_1_admin_csrf_token is missing else l_1_admin_csrf_token))
        yield '">\n                                    <textarea name="admin_prompt" rows="2" class="rounded-xl border border-white/10 bg-[#0d0f0f] px-3 py-2 text-xs text-[#e2e2e2] outline-none" placeholder="Optional admin prompt"></textarea>\n                                    <button type="submit" class="w-full rounded-xl bg-[#dec38f]/15 px-3 py-2 text-[10px] font-bold uppercase tracking-[0.18em] text-[#dec38f] border border-[#dec38f]/20 cursor-pointer">Regenerate With AI</button>\n                                </form>\n                                <form method="post" action="'
...
debug_info = '1=12&3=17&5=27&19=77&20=79&21=81&30=83&31=85&32=87&35=89&44=91&48=93&52=95&56=97&60=99&64=101&68=103&72=105&76=107&80=109&81=112&82=115&83=118&88=121&90=123&92=126&98=133&99=136&104=139&108=145&112=147&116=149&122=151&126=153&130=155&133=157&134=160&136=163&137=166&147=169&150=171&153=173&154=178&159=188&160=193&177=203&180=208&181=210&182=212&183=215&185=218&186=221&188=224&191=228&192=230&193=232&194=235&197=238&198=240&200=242&201=244&202=246&204=248&205=252&208=262&209=264&210=266&211=268&212=270&213=272&215=274&218=278&223=280&224=282&228=284&229=286&239=290&240=296&241=298&251=304&255=306&256=310&261=320&262=324&268=334&271=339&272=341&273=343&275=345&276=347&277=349&279=351&280=355&288=365&293=373&294=379&295=381&305=387&325=389&328=394&329=396&330=398&331=401&334=404&335=406&336=408&337=410&338=412&339=414&340=416&342=418&349=425&350=427&351=429&360=436&361=438&364=440&365=443&374=448&375=454&376=456'